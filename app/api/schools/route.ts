...
export async function POST(request: NextRequest) {
  try {
    ...
    const academic_programs = formData.get('academic_programs') as string;
    ...
    const total_students = formData.get('total_students') as string;
    const board_affiliation = formData.get('board_affiliation') as string;
    ...
    if (imageFile) {
      ...
      // In production, you should use a cloud storage service like Cloudinary, AWS S3, or similar
      const timestamp = Date.now();
      const fileExtension = imageFile.name.split('.').pop();
      const fileName = `school_${timestamp}.${fileExtension}`;
      
      // For now, use a default image path
      imagePath = `/schoolImages/default-school.jpg`;
    ...
    const result = await db.run(
      `INSERT INTO schools (
        ...
        about, academic_programs, facilities, website, 
        established_year, principal_name, total_students, board_affiliation
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
